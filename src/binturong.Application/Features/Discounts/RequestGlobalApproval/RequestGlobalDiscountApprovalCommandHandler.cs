...
internal sealed class RequestGlobalDiscountApprovalCommandHandler
    : ICommandHandler<RequestGlobalDiscountApprovalCommand, Guid>
{
    ...
    public RequestGlobalDiscountApprovalCommandHandler(
    ...
    public async Task<Result<Guid>> Handle(
        RequestGlobalDiscountApprovalCommand cmd,
    ...
    )
    {
        ...
        if (!so.Details.Any())
            return Result.Failure<Guid>(DiscountErrors.SalesOrderWithoutItems);
        ...
        var req = so.CreateApprovalRequestForGlobalDiscount(
            cmd.DiscountPerc,
            _currentUser.UserId,
            cmd.Reason,
            DateTime.UtcNow
        ...
        await _bus.AuditAsync(
            ...
            req.Id,
            "DISCOUNT_GLOBAL_APPROVAL_REQUESTED",
            string.Empty,
            $"salesOrderId={req.SalesOrderId}; perc={req.RequestedPercentage}",
            _request.IpAddress,
            _request.UserAgent,
            ct
        );

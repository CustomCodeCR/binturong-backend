...
internal sealed class GetSalesOrdersSelectQueryHandler
    : IQueryHandler<GetSalesOrdersSelectQuery, IReadOnlyList<SelectOptionDto>>
{
    private const string Module = "Sales";
    private const string Entity = "SalesOrder";
    ...
    public GetSalesOrdersSelectQueryHandler(
        IMongoDatabase db,
        ICommandBus bus,
    ...
    public async Task<Result<IReadOnlyList<SelectOptionDto>>> Handle(
        GetSalesOrdersSelectQuery query,
    ...
    private static string BuildLabel(SalesOrderReadModel x)
    {
        ...
        return x.SalesOrderId.ToString();
    }
